...
# Celery
# ─────────────────────────────────────────────
# FIX: Default to synchronous task execution (no Redis needed)
# This prevents crashes when Redis is not available
# Set CELERY_TASK_ALWAYS_EAGER=False and add Redis when ready for async
CELERY_BROKER_URL = os.getenv("CELERY_BROKER_URL", "redis://localhost:6379/0")
CELERY_RESULT_BACKEND = os.getenv("CELERY_RESULT_BACKEND", CELERY_BROKER_URL)
CELERY_ACCEPT_CONTENT = ["json"]
CELERY_TASK_SERIALIZER = "json"
CELERY_RESULT_SERIALIZER = "json"
# CELERY_TASK_ALWAYS_EAGER makes tasks run synchronously (no broker needed)
# Default to True to avoid Redis dependency errors on Railway
